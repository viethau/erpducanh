...
namespace DucAnhERP.Services
{
    ...
    public class PhanLoaiDeCongRepository : IPhanLoaiDeCongRepository
    {
        ...
        public async Task<List<PhanLoaiDeCong>> GetAll(string groupId)
        {
            try
            {
        ...
        public async Task<List<PhanLoaiDeCongModel>> GetAllByVM(PhanLoaiDeCongModel pldcModel)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = from pldc in context.PhanLoaiDeCongs
                            ...
                            select new PhanLoaiDeCongModel
                            {
                                Id = pldc.Id,
                                Flag = pldc.Flag,
                                IsEdit = context.DSNuocMua.Any(ds => ds.ThongTinDeCong_TenLoaiDeCong == pldc.Id) ? 1 : 0,
                ...
                if (!string.IsNullOrEmpty(pldcModel.ThongTinDuongTruyenDan_LoaiTruyenDan))
                {
                    query = query.Where(x => x.ThongTinDuongTruyenDan_LoaiTruyenDan == pldcModel.ThongTinDuongTruyenDan_LoaiTruyenDan);
                }
                if (!string.IsNullOrEmpty(pldcModel.ThongTinDeCong_CauTaoDeCong))
                {
                    query = query.Where(x => x.ThongTinDeCong_CauTaoDeCong == pldcModel.ThongTinDeCong_CauTaoDeCong);
                }
        ...
        public async Task<bool> CheckUsingName(string name)
        {
            ...
            var query = context.PhanLoaiDeCongs
                         .Where(item => (item.ThongTinDeCong_TenLoaiDeCong.ToUpper().Trim() == name.ToUpper().Trim()));
        ...
        public async Task Update(PhanLoaiDeCong PhanLoaiDeCong, string userId)
        {
            using var context = _context.CreateDbContext();
        ...
        public async Task Insert(PhanLoaiDeCong entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
