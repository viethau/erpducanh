...
{
    public class PKKLTDanCHopRepository :IPKKLTDanCHopRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;

        public PKKLTDanCHopRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        ...
        public async Task<List<PKKLTDanCHop>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.PKKLTDanCHops.ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<PKKLTDanCHop> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.PKKLTDanCHops.Where(x => x.Id.Equals(id)).FirstOrDefaultAsync();
        ...
        public async Task<List<THKLModel>> GetTHKLTDanCHop()
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from a in context.PKKLTDanCHops
                             join b in context.PhanLoaiTDanTDans
                             on a.TTTDCongHoRanh_TenLoaiTamDanTieuChuan equals b.Id
                             let kl1Dv = context.PKKLTDanCHops
        ...
        public async Task<List<THKLModel>> GetTHKLByTuyenDuong(List<NuocMuaModel> nuocMua)
        {
            try
            {
                ...
                foreach (var item in nuocMua)
                {
                    ...
                    var query = await (from a in context.PKKLTDanCHops
                                       join b in context.PhanLoaiTDanTDans
                                           on a.TTTDCongHoRanh_TenLoaiTamDanTieuChuan equals b.Id
        ...
        {
            using var context = _context.CreateDbContext();
            var result = context.PKKLTDanCHops
        ...
        public async Task<List<PKKLTDanCHop>> GetExist(PKKLTDanCHop searchData)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                var query = context.PKKLTDanCHops
                             .Where(item => (
        ...
        public async Task Update(PKKLTDanCHop TKThepDeCong, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.PKKLTDanCHops.Update(TKThepDeCong);
            await SaveChanges(context);
        }
        public async Task UpdateMulti(PKKLTDanCHop[] PKKLTDanCHop)
        {
            ...
            try
            {
                ...
                string[] ids = PKKLTDanCHop.Select(x => x.Id).ToArray();

                // Lấy danh sách các bản ghi từ database có ID nằm trong danh sách ids
                var listEntities = await context.PKKLTDanCHops
                    .Where(x => ids.Contains(x.Id))
                    .ToListAsync();
                foreach (var entity in listEntities)
                {
                    var updatedEntity = PKKLTDanCHop.FirstOrDefault(x => x.Id == entity.Id);
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<PKKLTDanCHop>().Remove(entity);
            await SaveChanges(context);
        }
        ...
        public async Task Insert(PKKLTDanCHop entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                var maxFlagValue = await context.PKKLTDanCHops
                ...
                context.PKKLTDanCHops.Add(entity);
                await SaveChanges(context);
            }
            catch (Exception ex)
            {
        ...
        public async Task<string> InsertLaterFlag(PKKLTDanCHop entity, int FlagLast, bool insertBefore)
        {
            ...
            try
            {
                ...
                if (recordsToUpdate.Count() == 0)
                {
                    ...
                    var maxFlag = await context.PKKLTDanCHops.AnyAsync()
                                  ? await context.PKKLTDanCHops.MaxAsync(x => x.Flag)
                                  : 0;
        ...
        public async Task SaveChanges(ApplicationDbContext context)
        {
            try
            {
                ...
                var entries = context.ChangeTracker.Entries<PKKLTDanCHop>();
        ...
        private async Task HandleEntityAdd(EntityEntry<PKKLTDanCHop> entry)
        {
            var entity = entry.Entity;
            if (entity.HangMuc == "I.Bê tông, vận chuyển, lắp đặt")
            {
                using var context = _context.CreateDbContext();
                if (entity.LoaiBeTong.ToUpper().Trim() == "Bê tông thương phẩm".ToUpper().Trim() && entity.HangMucCongTac.Trim().ToLower() == "Tấm đan cống hộp".Trim().ToLower())
                {
                    //Cập nhật I.
                    var recordsToUpdate = await context.PKKLTDanCHops
                    ...
                    foreach (var record in recordsToUpdate)
                    {
                        UpdateRecordWithCalculations(record, entity.TKLCK_SauCC); // Xử lý giá trị sau khi xóa
                    }
                    await UpdateMulti(recordsToUpdate.ToArray());
                }

                //Cập nhật II.
                var recordsToUpdate1 = await context.PKKLTDanCHops
                    .Where(x => x.TTTDCongHoRanh_TenLoaiTamDanTieuChuan == entity.TTTDCongHoRanh_TenLoaiTamDanTieuChuan
        ...
        private async Task HandleEntityUpdate(EntityEntry<PKKLTDanCHop> entry)
        {
            var entity = entry.Entity;
            if (entity != null)
            {
        ...
        private async Task HandleEntityDelete(EntityEntry<PKKLTDanCHop> entry)
        {
            var entity = entry.Entity;
            if (entity.HangMuc == "I.Bê tông, vận chuyển, lắp đặt")
            {
        ...
        //{
        //    var addedEntity = entityEntry.Entity as PKKLTDanCHop;
        //    if (addedEntity != null)
        //    {
        ...
        //                if (!string.IsNullOrEmpty(record.TTTDCongHoRanh_TenLoaiTamDanTieuChuan))
        //                {
        //                    var TKLCK_SauCC1 = await GetTKLCK_SauCCByLCK(record.TTTDCongHoRanh_TenLoaiTamDanTieuChuan);
        ...
        //{
        //    var modifiedEntity = entityEntry.Entity as PKKLTDanCHop;
        ...
        //{
        //    var deletedEntity = entityEntry.Entity as PKKLTDanCHop;
        ...
        //tính toán
        private void UpdateRecordWithCalculations(PKKLTDanCHop record, double tklckSauCc)
        {
        ...
        public double KTHH_KL1CK(PKKLTDanCHop obj)
        {
            double result = 0;
            if (obj.DonVi == "M3")
            {
        ...
        public double TTCDT_KL(PKKLTDanCHop obj)
        {
            double result = 0;
        ...
        public double KL1CK_ChuaTruCC(PKKLTDanCHop obj)
        {
            double result = 0;
            if (obj.KTHH_KL1CK > 0)
            {
