...
namespace DucAnhERP.Services
{
    public class TKThepCTronRepository : ITKThepCTronRepository
    {
        ...
        public async Task<string> InsertLaterFlag(TKThepCTron entity, int FlagLast, bool insertBefore)
        {
            string id = "";
            try
            {
                ...
                if (result != null)
                {
                    ...
                    foreach (var record in result)
                    {
                        ...
                        if (!string.IsNullOrEmpty(record.TenCongTac))
                        {
                            ...
                            pKKLCTron.KTHH_KL1CK = _pKKLCTronRepository.KTHH_KL1CK(pKKLCTron);
                            pKKLCTron.TTCDT_KL = _pKKLCTronRepository.TTCDT_KL(pKKLCTron);
                            pKKLCTron.KL1CK_ChuaTruCC = _pKKLCTronRepository.KL1CK_ChuaTruCC(pKKLCTron);
