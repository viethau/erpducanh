...
namespace DucAnhERP.Services
{
    public class PKKLCHopRepository : IPKKLCHopRepository
    {
        ...
        public async Task<List<PKKLCHop>> GetAll(string groupId)
        {
            try
            {
        ...
        public async Task Update(PKKLCHop TKThepDeCong, string userId)
        {
            using var context = _context.CreateDbContext();
        ...
        public async Task Insert(PKKLCHop entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<string> InsertLaterFlag(PKKLCHop entity, int FlagLast, bool insertBefore)
        {
            string id = "";
            try
            {
        ...
        public async Task SaveChanges(ApplicationDbContext context)
        {
            try
            {
                ...
                var entries = context.ChangeTracker.Entries<PKKLCHop>();
        ...
        private async Task HandleEntityAdd(EntityEntry<PKKLCHop> entry)
        {
            ...
            if (entity != null)
            {
                ...
                {
                    using var context = _context.CreateDbContext();
                    if (entity.HangMucCongTac.Trim().ToLower() == "Bê tông cống hộp".Trim().ToLower())
                    {
                        //Cập nhật I.
                        var recordsToUpdate = await context.PKKLCHops
        ...
        private async Task HandleEntityUpdate(EntityEntry<PKKLCHop> entry)
        {
            var entity = entry.Entity;
            if (entity != null)
            {
        ...
        private async Task HandleEntityDelete(EntityEntry<PKKLCHop> entry)
        {
            var entity = entry.Entity;
            if (entity != null)
            {
        ...
        //tính toán
        private void UpdateRecordWithCalculations(PKKLCHop record, double tklckSauCc)
        {
        ...
        public double KTHH_KL1CK(PKKLCHop obj)
        {
            double result = 0;
            if (obj.DonVi == "M3")
            {
        ...
        public double TTCDT_KL(PKKLCHop obj)
        {
            double result = 0;
        ...
        public double KL1CK_ChuaTruCC(PKKLCHop obj)
        {
            double result = 0;
            if (obj.KTHH_KL1CK > 0)
            {
