...
{
    public class PKKLRanhBTRepository :IPKKLRanhBTRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;

        public PKKLRanhBTRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        ...
        public async Task<List<PKKLRanhBT>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.PKKLRanhBTs.ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<PKKLRanhBT> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.PKKLRanhBTs.Where(x => x.Id.Equals(id)).FirstOrDefaultAsync();
        ...
        public async Task<List<THKLModel>> GetTHKLRanhBT()
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        {
            using var context = _context.CreateDbContext();
            var result = context.PKKLRanhBTs.Where(x => new[] { "I.Móng", "II.Tường", "III.Mũ mố" }.Contains(x.HangMuc) &&
        ...
        public async Task Update(PKKLRanhBT TKThepDeCong, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.PKKLRanhBTs.Update(TKThepDeCong);
            await SaveChanges(context);
        }
        public async Task UpdateMulti(PKKLRanhBT[] PKKLRanhBT)
        {
            try
            {
                using var context = _context.CreateDbContext();

                string[] ids = PKKLRanhBT.Select(x => x.Id).ToArray();

                // Lấy danh sách các bản ghi từ database có ID nằm trong danh sách ids
                var listEntities = await context.PKKLRanhBTs
                    .Where(x => ids.Contains(x.Id))
                    .ToListAsync();
                foreach (var entity in listEntities)
                {
                    var updatedEntity = PKKLRanhBT.FirstOrDefault(x => x.Id == entity.Id);
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<PKKLRanhBT>().Remove(entity);
            await SaveChanges(context);
        }
        ...
        public async Task Insert(PKKLRanhBT entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                var maxFlagValue = await context.PKKLRanhBTs
                ...
                context.PKKLRanhBTs.Add(entity);
                await SaveChanges(context);
            }
            catch (Exception ex)
            {
        ...
        public async Task<string> InsertLaterFlag(PKKLRanhBT entity, int FlagLast, bool insertBefore)
        {
            string id = "";
            try
            {
                ...
                if (recordsToUpdate.Count() == 0)
                {
                    ...
                    var maxFlag = await context.PKKLRanhBTs.AnyAsync()
                                  ? await context.PKKLRanhBTs.MaxAsync(x => x.Flag)
                                  : 0;
        ...
        public async Task SaveChanges(ApplicationDbContext context)
        {
            try
            {
                ...
                var entries = context.ChangeTracker.Entries<PKKLRanhBT>();
        ...
        private async Task HandleEntityAdd(EntityEntry<PKKLRanhBT> entry)
        {
            var entity = entry.Entity;
            if (entity != null)
            {
                // Kiểm tra điều kiện HangMuc và PhanLoaiMongCongTronCongHop
                if (entity.HangMuc == "I.Móng" || entity.HangMuc == "II.Tường" || entity.HangMuc == "III.Mũ mố" 
                    && entity.HangMucCongTac.Trim().ToLower() == "Bê tông thương phẩm".Trim().ToLower())
                {
                    using var context = _context.CreateDbContext();
        ...
        private async Task HandleEntityUpdate(EntityEntry<PKKLRanhBT> entry)
        {
            var entity = entry.Entity;
            if (entity != null)
            {
                // Kiểm tra điều kiện HangMuc
                if (entity.HangMuc == "I.Móng" || entity.HangMuc == "II.Tường" || entity.HangMuc == "III.Mũ mố" 
                    && entity.HangMucCongTac.Trim().ToLower() == "Bê tông thương phẩm".Trim().ToLower())
                {
                    using var context = _context.CreateDbContext();
        ...
        private async Task HandleEntityDelete(EntityEntry<PKKLRanhBT> entry)
        {
            var entity = entry.Entity;
            if (entity != null)
            {
        ...
        //tính toán
        private void UpdateRecordWithCalculations(PKKLRanhBT record, double tklckSauCc)
        {
        ...
        public double KTHH_KL1CK(PKKLRanhBT obj)
        {
            double result = 0;
            if (obj.DonVi == "M3")
            {
        ...
        public double TTCDT_KL(PKKLRanhBT obj)
        {
            double result = 0;
        ...
        public double KL1CK_ChuaTruCC(PKKLRanhBT obj)
        {
            double result = 0;
            if (obj.KTHH_KL1CK > 0)
            {
