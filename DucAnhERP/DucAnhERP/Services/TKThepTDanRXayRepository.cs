...
namespace DucAnhERP.Services
{
    public class TKThepTDanRXayRepository : ITKThepTDanRXayRepository
    {
        ...
        public async Task<List<TKThepTDanRXay>> GetAll(string groupId)
        {
            try
            {
        ...
        public async Task Update(TKThepTDanRXay TKThepTDanRXay, string userId)
        {
            using var context = _context.CreateDbContext();
        ...
        public async Task UpdateMulti(TKThepTDanRXay[] tKThepTDanRXayArray)
        {
            using var context = _context.CreateDbContext();

            // Lấy danh sách ID từ mảng đầu vào
            var ids = tKThepTDanRXayArray.Select(x => x.Id).ToArray();

            // Lấy danh sách thực thể từ cơ sở dữ liệu dựa trên ID
            var listEntities = await context.TKThepTDanRXays
            ...
            foreach (var entity in listEntities)
            {
                var updatedEntity = tKThepTDanRXayArray.FirstOrDefault(x => x.Id == entity.Id);
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            // Bước 2: Xóa bản ghi
            context.TKThepTDanRXays.Remove(entity);
            await SaveChanges(context);
        }
        ...
        public async Task Insert(TKThepTDanRXay entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<string> InsertLaterFlag(TKThepTDanRXay entity, int FlagLast, bool insertBefore)
        {
            string id = "";
            try
            {
                ...
                if (result != null)
                {
                    ...
                    foreach (var record in result)
                    {
                        ...
                        if (!string.IsNullOrEmpty(record.TenCongTac))
                        {
                            ...
                            PKKLTDanRXay.KTHH_KL1CK = _pKKLTDanRXayRepository.KTHH_KL1CK(PKKLTDanRXay);
                            PKKLTDanRXay.TTCDT_KL = _pKKLTDanRXayRepository.TTCDT_KL(PKKLTDanRXay);
                            PKKLTDanRXay.KL1CK_ChuaTruCC = _pKKLTDanRXayRepository.KL1CK_ChuaTruCC(PKKLTDanRXay);
                            PKKLTDanRXay.TKLCK_SauCC = Math.Round((PKKLTDanRXay.KL1CK_ChuaTruCC - PKKLTDanRXay.KLCC1CK), 4);
