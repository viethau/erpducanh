    ...
    public class PKKLCTronRepository : IPKKLCTronRepository
    {
        ...
        public async Task UpdateMulti(PKKLCTron[] PKKLCTron)
        {
            ...
            try
            {
                ...
                string[] ids = PKKLCTron.Select(x => x.Id).ToArray();

                // Lấy danh sách các bản ghi từ database có ID nằm trong danh sách ids
                var listEntities = await context.PKKLCTrons
                    .Where(x => ids.Contains(x.Id))
                    .ToListAsync();
                foreach (var entity in listEntities)
                {
                    var updatedEntity = PKKLCTron.FirstOrDefault(x => x.Id == entity.Id);
        ...
        public async Task SaveChanges(ApplicationDbContext context)
        {
            try
            {
                ...
                var entries = context.ChangeTracker.Entries<PKKLCTron>();
        ...
        private async Task HandleEntityAdd(EntityEntry<PKKLCTron> entry)
        {
            var entity = entry.Entity;
            if (entity.HangMuc == "I.Sản xuất, vận chuyển, lắp đặt" && entity.LoaiBeTong.Trim().ToLower() == "Bê tông thương phẩm".Trim().ToLower())
            {
                using var context = _context.CreateDbContext();

                if(entity.HangMucCongTac.Trim().ToLower() == "Bê tông cống tròn".Trim().ToLower() && entity.KTHH_GhiChu == "Rộng*Cao")
                {
                    //Cập nhật I.
                    var recordsToUpdate = await context.PKKLCTrons
        ...
        private async Task HandleEntityUpdate(EntityEntry<PKKLCTron> entry)
        {
            var entity = entry.Entity;
        ...
        private async Task HandleEntityDelete(EntityEntry<PKKLCTron> entry)
        {
            var entity = entry.Entity;
            if (entity.HangMuc == "I.Sản xuất, vận chuyển, lắp đặt" && entity.LoaiBeTong.Trim().ToLower() == "Bê tông thương phẩm".Trim().ToLower())
            {
        ...
        public double KTHH_KL1CK(PKKLCTron obj)
        {
            double result = 0;
            if (obj.DonVi == "M3")
            {
        ...
        public double TTCDT_KL(PKKLCTron obj)
        {
            double result = 0;
        ...
        public double KL1CK_ChuaTruCC(PKKLCTron obj)
        {
            double result = 0;
            if (obj.KTHH_KL1CK > 0)
            {
