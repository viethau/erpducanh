...
namespace DucAnhERP.Services
{
    public class TKThepTChongRepository : ITKThepTChongRepository
    {
        ...
        public async Task<List<TKThepTChong>> GetAll(string groupId)
        {
            try
            {
        ...
        public async Task Update(TKThepTChong TKThepDeCong, string userId)
        {
            using var context = _context.CreateDbContext();
        ...
        //}
        public async Task UpdateMulti(TKThepTChong[] TKThepTChongArray)
        {
            using var context = _context.CreateDbContext();

            // Lấy danh sách ID từ mảng đầu vào
            var ids = TKThepTChongArray.Select(x => x.Id).ToArray();

            // Lấy danh sách các thực thể từ cơ sở dữ liệu
            var listEntities = await context.TKThepTChongs
            ...
            foreach (var entity in listEntities)
            {
                ...
                var updatedEntity = TKThepTChongArray.FirstOrDefault(x => x.Id == entity.Id);
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            await context.TKThepTChongs
                .Where(x => x.Flag > entity.Flag &&
                            x.TTKTHHCongHopRanh_LoaiThanhChong == entity.TTKTHHCongHopRanh_LoaiThanhChong)
                .ExecuteUpdateAsync(x => x
                    .SetProperty(r => r.Flag, r => r.Flag - 1)
                    .SetProperty(r => r.SoHieu, r => r.SoHieu - 1));

            // Bước 2: Xóa bản ghi
            context.TKThepTChongs.Remove(entity);
            await SaveChanges(context);
        }
        ...
        public async Task Insert(TKThepTChong entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<string> InsertLaterFlag(TKThepTChong entity, int FlagLast, bool insertBefore)
        {
            string id = "";
            try
            {
