...
namespace DucAnhERP.Services
{
    public class PKKLMongCTronRepository :IPKKLMongCTronRepository
    {
        ...
        //        using var context = _context.CreateDbContext();
        //        var query = (from a in context.PKKLMongCTrons
        //                     join b in context.PhanLoaiCTronHopNhuas on a.ThongTinMongDuongTruyenDan_PhanLoaiMongCongTronCongHop equals b.Id
        ...
        //                         PhanLoaiCTronHopNhua_TenLoaiTruyenDanSauPhanLoai = c.ThongTinMongDuongTruyenDan_PhanLoaiMongCongTronCongHop,
        //                         a.TenCongTac,
        //                         a.DonVi,
        //                         a.TKLCK_SauCC,
        //                         a.HangMuc,
        ...
        //                     {
        //                         Id = g.Key.Id,
        //                         ThongTinMongDuongTruyenDan_PhanLoaiMongCongTronCongHop = g.Key.ThongTinMongDuongTruyenDan_PhanLoaiMongCongTronCongHop,
        ...
        //                                   ThongTinMongDuongTruyenDan_PhanLoaiMongCongTronCongHop = g.Key.ThongTinMongDuongTruyenDan_PhanLoaiMongCongTronCongHop,
        //                                   PhanLoaiCTronHopNhua_TenLoaiTruyenDanSauPhanLoai = g.Key.ThongTinMongDuongTruyenDan_PhanLoaiMongCongTronCongHop, // Cố định tên loại từ SQL
        //                                   TenCongTac = g.Key.TenCongTac,
        //                                   DonVi = g.Key.DonVi,
        //                                   KL1DonVi = g.Key.TKLCK_SauCC,
        //                                   SLTrai = g.Sum(x => x.c != null && x.c.TraiPhai == 0 ? x.c.TTCDSLCauKienDuongTruyenDan_SlCauKienTinhKl : 0) ?? 0,
        ...
        public async Task<PKKLMongCTron> GetTKLCK_SauCCByLCK(string id)
            ...
            public async Task UpdateMulti(PKKLMongCTron[] TKThepDeCong)
            {
                using var context = _context.CreateDbContext();
                string[] ids = TKThepDeCong.Select(x => x.Id).ToArray();
                var listEntities = await context.PKKLMongCTrons.Where(x => ids.Contains(x.Id)).ToListAsync();
                foreach (var entity in listEntities)
                {
                    context.PKKLMongCTrons.Update(entity);
                }
                await context.SaveChangesAsync();
            }
