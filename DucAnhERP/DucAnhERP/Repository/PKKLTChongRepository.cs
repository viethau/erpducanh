    ...
    public class PKKLTChongRepository :IPKKLTChongRepository
    {
        ...
        public async Task Insert(PKKLTChong entity)
        {
            try
            {
                ...
                var maxFlagValue = await context.PKKLTChongs
        ...
        {
            var addedEntity = entityEntry.Entity as PKKLTChong;
            if (addedEntity != null)
            {
                // Kiểm tra điều kiện HangMuc và PhanLoaiMongCongTronCongHop
                if (addedEntity.HangMuc == "I.Bê tông, vận chuyển, lắp đặt"
                    ...
                    && addedEntity.HangMucCongTac.Trim().ToLower() == "Bê tông cống tròn".Trim().ToLower())
                {
                    ...
                    // Lọc tất cả các bản ghi có TTKTHHCongHopRanh_LoaiThanhChong giống với addedEntity
                    var recordsToUpdate = await context.PKKLTChongs
                        .Where(x => x.TTKTHHCongHopRanh_LoaiThanhChong == addedEntity.TTKTHHCongHopRanh_LoaiThanhChong
                    ...
                    foreach (var record in recordsToUpdate1)
                    {
                        ...
                        {
                            var TKLCK_SauCC1 = await GetTKLCK_SauCCByLCK(record.TTKTHHCongHopRanh_LoaiThanhChong);
        ...
        {
            var modifiedEntity = entityEntry.Entity as PKKLTChong;
        ...
        {
            var deletedEntity = entityEntry.Entity as PKKLTChong;
            if (deletedEntity != null)
            {
                ...
                if (deletedEntity.HangMuc == "I.Bê tông, vận chuyển, lắp đặt")
                {
                    ...
                    && deletedEntity.HangMucCongTac.Trim().ToLower() == "Bê tông cống tròn".Trim().ToLower())
                    {
                        // Lọc tất cả các bản ghi có TTKTHHCongHopRanh_LoaiThanhChong giống với addedEntity
                        var recordsToUpdate = await context.PKKLTChongs
                            .Where(x => x.TTKTHHCongHopRanh_LoaiThanhChong == deletedEntity.TTKTHHCongHopRanh_LoaiThanhChong
                            && x.HangMuc == "I.Bê tông, vận chuyển, lắp đặt" && x.HangMucCongTac == "Vận chuyển từ bãi đúc đến công trường")
                            .ToListAsync();
